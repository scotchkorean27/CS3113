...
void Player::Update(unsigned int** tiles){
	if (isactive()){
		lcol = false;
		rcol = false;
		ucol = false;
...
void Player::xwallcols(unsigned int** walls){
	pair<int, int> left;
	pair<int, int> right;
	left = worldToTileCoordinates(lx(), gety());
	right = worldToTileCoordinates(rx(), gety());
	if (walls[right.second][right.first] > 0){
		...
		float test = (right.first) * TILE_SIZE - (TILE_SIZE * 128 / 2);
		float xpen = fabs(rx() - test);
		move(-xpen - 0.001, 0);
		hspeed = 0;
		rcol = true;
	}
...
void Player::ywallcols(unsigned int** walls){
	pair<int, int> up;
	pair<int, int> down;
...
Enemy::Enemy(int tex, float xp, float yp)
: Entity(tex, xp, yp, 0.075, 0.075),
aniframe(0),
anind(0),
vspeed(0),
hspeed(0),
